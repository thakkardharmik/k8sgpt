...
type HTTPSOnlyServiceV2Analyzer struct{}

func (analyzer HTTPSOnlyServiceV2Analyzer) Analyze(a common.Analyzer) ([]common.Result, error) {
	kind := "Service"
	...
	for _, service := range serviceList.Items {
		...
		nginxSSLConfigEnabled := false
		if service.Annotations["nginx.ingress.kubernetes.io/ssl-redirect"] == "true" {
			nginxSSLConfigEnabled = true
		}

		for _, port := range service.Spec.Ports {
			if (port.Port == 80 || strings.Contains(strings.ToLower(port.Name), "http")) && !nginxSSLConfigEnabled {
				doc := apiDoc.GetApiDocV2("spec.ports")
				failures = append(failures, common.Failure{
					Text:          fmt.Sprintf("Service %s is accessible over HTTP on port %d without enforced SSL in Nginx config", service.Name, port.Port),
					KubernetesDoc: doc,
